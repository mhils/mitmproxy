...
class AddonManager:
    ...
    def _configure_all(self, options, updated):
        self.trigger_sync(hooks.ConfigureHook(updated))
    ...
    async def remove_async(self, addon):
        """
        ...
        await self.invoke_addon_async(addon, hooks.DoneHook())
    ...
    async def handle_lifecycle(self, event: hooks.Hook):
        ...
        await self.trigger_async(event)

        if isinstance(message, flow.Flow):
            await self.trigger_async(hooks.UpdateHook([message]))

    def _invoke_addon(self, addon, event: hooks.Hook):
        """
    ...
    async def invoke_addon_async(self, addon, event: hooks.Hook):
        for awaitable in self._invoke_addon(addon, event):
            await awaitable

    def invoke_addon_sync(self, addon, event: hooks.Hook):
        for _ in self._invoke_addon(addon, event):
            ctx.log.error(
                f"Addon error: {addon}'s {event.name} returned an awaitable, "
                f"but the event must be synchronous.'"
            )
    ...
    async def trigger_async(self, event: hooks.Hook):
        ...
        for i in self.chain:
            try:
                with safecall():
                    await self.invoke_addon_async(i, event)
            except exceptions.AddonHalt:
                return

    def trigger_sync(self, event: hooks.Hook):
        """
            Trigger an event across all addons.
        """
        for i in self.chain:
