...
import socket
import time
from asyncio import staggered, CancelledError
...
class ConnectionHandler(metaclass=abc.ABCMeta):
        ...
        async with self.max_conns[command.connection.address]:
            ...
            try:
                ...
                if is_ip_address(command.connection.address[0]):
                    ...
                    command.connection.timestamp_dns_resolved = command.connection.timestamp_start
                else:
                    if resolver := dns_resolver_fixme.resolver():  # FIXME: reuse shared instance
                ...
                if command.connection.transport_protocol == "tcp":
                    reader, writer = await self.open_tcp_connection(ips, port, local_addr=command.connection.sockname)
                elif command.connection.transport_protocol == "udp":
                    reader = writer = await self.open_udp_connection(ips, port, local_addr=command.connection.sockname)
                else:
    ...
    async def open_udp_connection(
        ...
        for ip in ips:
            try:
                ...
                continue
        if stream is None:
            raise OSError(f"connection establishment failed: {'; '.join({str(e) for e in exceptions})}")
        return stream
    ...
    async def open_tcp_connection(
        ...
        for ip in ips:
            try:
                sock = await self._connect_sock(ip=ip, port=port, type=socket.SOCK_STREAM, local_addr=local_addr)
                break
            except OSError as e:
    ...
    async def _connect_sock(self, ip: str, port: int, type: int, local_addr: tuple[str, int] | None):
        if ":" in ip:
